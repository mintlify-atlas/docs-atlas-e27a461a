...
description: "Stacked disclosure panels with muted-to-active header color, semantic border tokens, and a CSS transition override for instant panel open/close."
---

The **Accordion** component renders a vertically stacked set of collapsible panels. The Nettalco preset styles headers with a muted default color that shifts to `{text.color}` on hover or when active. A deliberate `transition: none` CSS override is applied to `.p-accordion-header-content` and `.p-accordion-content` to keep expansion and collapse instantaneous — matching application environments where animation-free UIs are preferred.
...
|---|---|---|
| `root.transitionDuration` | `{transition.duration}` | Duration used for internal CSS transitions (where not overridden — see Theme Notes) |
...
- **`transition: none` override** — The Nettalco preset sets `transition: none` directly on `.p-accordion-header-content` and `.p-accordion-content` via component-level CSS. This means panel expansion/collapse is instantaneous even though `root.transitionDuration` is defined. The token is kept for consistency with the PrimeNG type contract but has no visual effect in this preset.
